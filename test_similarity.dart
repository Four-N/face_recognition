...
void main() {
  ...
  print(
    'Registered vector (first 5 elements): ${registeredVector.take(5).toList()}',
  );
  print(
    'Registered vector norm: ${math.sqrt(registeredVector.fold(0.0, (sum, val) => sum + val * val))}',
  );
  ...
  norm = math.sqrt(verifyVector.fold(0.0, (sum, val) => sum + val * val));
  if (norm > 0) {
  ...
  print(
    'Verify vector norm: ${math.sqrt(verifyVector.fold(0.0, (sum, val) => sum + val * val))}',
  );
  ...
  norm = math.sqrt(differentVector.fold(0.0, (sum, val) => sum + val * val));
  if (norm > 0) {
  ...
  final differentSimilarity = calculateSimilarity(
    registeredVector,
    differentVector,
  );
  print(
    'Similarity with different vector: ${(differentSimilarity * 100).toStringAsFixed(1)}%',
  );

  // Test 5: Test with identical vectors
  final identicalSimilarity = calculateSimilarity(
    registeredVector,
    registeredVector,
  );
  print(
    'Similarity with identical vector: ${(identicalSimilarity * 100).toStringAsFixed(1)}%',
  );
}
